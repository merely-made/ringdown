...
- **Provenance discipline.** Every protocol fact in a plan's Findings cites
  where it was read — a decompiled class path, a live capture, or an on-device
  probe — and marks its confidence (static-read, fixture-verified, or
  hardware-verified). A static read is a hypothesis until a probe confirms it
  against the actual instrument; do not promote one to fact without the control
  run. This is the house "prove the instrument" rule applied to a protocol.
- **Expression boundary.** This repo carries no vendor code, no decompiled
  sources, and no vendor trademark in a crate name. Findings record *facts and
  interfaces* — UUIDs, field names, wire formats, state machines — and
  paraphrase behaviour, citing class names for provenance only; they never
  paste vendor source. The vendor's firmware binaries are never redistributed
  here. The line this draws is the one copyright itself draws: interfaces and
  functional facts are not protectable expression, the vendor's actual code is.
- **Say "reverse-engineered", not "clean-room".** These are different claims and
  ...
  would be an overclaim in the one place accuracy matters most. If a genuine
  split is ever wanted, the Findings section is already the spec: hand it to an
  implementer who never opens the teardown, and record that separation here.

Docs are flat in `design_docs/` today. The likely first area roots under core
§4, should the project accumulate them, are `protocol_docs/` and, if firmware
is ever entered, `firmware_docs/`.
